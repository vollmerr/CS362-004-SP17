...
void test_effect_adventurer(struct gameState *actual, struct gameState *expected, int player, int hand_count) {
  int ret;
  // int expected_treasure=2;
  // int expected_handCount=actual->handCount[player] + 2;
  // printf("BEFORE: actual->handCount[player]: %d\n", actual->handCount[player]);
  // print_(actual->deck[player],actual->deckCount[player],"DECK before");
  // ret = effect_adventurer(actual);
  // print_(actual->deck[player],actual->deckCount[player],"DECK after");
  // for (i=0; i<expected_handCount; i++) {
  //   if (actual->hand[player][i] == copper ||
  //       actual->hand[player][i] == silver ||
  //       actual->hand[player][i] == gold) {
  //     expected_treasure--;
  //   }
  // }
  // printf("number of cards drawn: %d\n", expected_treasure);
  // printf("AFTER: actual->handCount[player]: %d\n", actual->handCount[player]);
  // test_result(!ret, "the correct return value");
  // test_result(!expected_treasure, "the correct number of treasure cards drawn");
  // test_result(expected_handCount == actual->handCount[player], "added only 2 cards to players hand");
